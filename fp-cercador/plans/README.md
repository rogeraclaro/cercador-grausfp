...
| 043 | [SPIKE — F5] Investigació fonts C LOMLOE→D i B→C LOE — reorientat 2026-09-06 a B LOMLOE→C LOMLOE (`plans/outputs/spike_043_results.md`: VIABLE, 99 % cobertura) | P2 | M | 042 DONE | DONE (C LOMLOE→D resolt 2026-09-06: `plans/outputs/spike_c_lomloe_d_results.md`, VIABLE 381/400) |
...
| 059 | FPO (SOC Catalunya): `soc_scraper.py` (Algolia `query` paginat dels 3 índexs), pipeline no-fatal, `/api/fpo/*`, historial i avís admin | P3 | M | — | DONE (desplegat 2026-09-06; snapshot 1.035 cursos / 2.974 especialitats / 2.708 centres en ~2s; 220 especialitats amb ≥1 curs, 720 cursos són cert. prof.; l'endpoint `browse` d'Algolia dona 403 amb la clau pública → es pagina amb `query`+`page`) |
| 060 | FPO: tercer mode "Cursos FPO (Catalunya)" al cercador (filtres Família→Àrea→Especialitat + comarca→municipi), panell de detall, bloc a la fitxa dels C de pla antic, secció a `fonts.html` | P3 | M–L | 059 DONE | DONE (validat amb server local + snapshot SOC; 220 especialitats, dependents OK, panell amb cursos/centres/badges, deep-link `?mode=fpo&esp=`, bloc "També s'ofereix…" als C de pla antic amb ≥1 curs FPO, CA/ES; cap canvi de backend) |
| 061 | FPO: favorits (especialitat + cursos, migracions 009/010), pestanya "Especialitats FPO" a perfil, targeta + refresc a admin | P3 | M | 059, 060 DONE | DONE (TDD backend: migracions 009/010, `/api/fpo/favorites*` amb sessió, `POST /api/admin/refresh-fpo` async + `GET /api/admin/fpo-status`; pestanya perfil + ganxo ⭐/checkbox al cercador + targeta admin; validat amb server local + BD de test; CA/ES) |
| 062 | FPO extern: scrapers de PIMEC (endpoint AJAX, paginat fins a pàgina buida, àrea per escaneig) i Foment (sitemap sense fragments, edicions), `ext_cursos.json`, bloc no fatal al pipeline amb snapshot segur per font | P3 | M | 059 DONE | DONE (TDD, 25 tests nous sense xarxa; verificat en real 2026-09-25: PIMEC 417 cursos, Foment 142 edicions; no desplegat) |
| 063 | FPO extern: especialitats sintètiques (`fpo_ext.py`), família per àrea/codi de certificat (fallback FCO), estat `finalitzat` derivat en llegir, `/api/fpo/*`, favorits sense migració, hook a `refresh-fpo` | P3 | M | 062 | DONE (TDD, 23 tests nous, 4 commits; cobertura d'àrees verificada amb dades reals 2026-09-25; no desplegat) |
| 064 | FPO extern: insígnies de font/tipus, filtres Font i Tipus, i18n CA/ES, perfil, `fonts.html`, `historial.html`; desplegament | P3 | M | 062, 063 | DONE (sense suite de tests; verificat amb Playwright sobre dades reals 2026-09-25: Font/Tipus, files i edicions externes, perfil, fonts i historial en CA/ES, 0 errors de consola; desplegat a producció 2026-09-25/26 amb refresc d'admin) |
...
| 066 | FPO extern: Cecot (`/courses/list`, 25 cursos) i CCOO/Fundació Paco Puerto (API REST WordPress, 70 cursos FPO) a `ext_cursos.json`; família explícita (`familiaCodi`); insígnies, i18n i `fonts.html` | P3 | M | 062, 063, 064 | DONE (TDD, 14 tests nous + ajustos a test_ext_refresh; verificat en real 2026-09-26: Cecot 25, CCOO 70 (61 especialitats), UI i fonts/historial en CA/ES, 0 errors de consola; pendent de desplegar) |
| 067 | FPO: paginar el llistat del mode «Cursos FPO» com fa la part de graus A–E (`pageSize`, `buildPagination`, controls i textos reutilitzats). Ara pinta totes les files de cop (~630; ~730 amb Cecot i CCOO). Cal revisar l'expandit de files, el `?esp=` i el reset de pàgina en filtrar | P3 | S | 064 | DONE (només frontend: `fpoPage` propi, 50 per pàgina, mateixos controls i textos que graus; verificat amb Playwright sobre 749 files en CA/ES, `?esp=` i mode graus intactes, 0 errors de consola; pendent de desplegar) |
